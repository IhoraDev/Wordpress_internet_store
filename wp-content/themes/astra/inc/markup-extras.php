 ...
 * @package     Astra
 * @author      Astra
 * @copyright   Copyright (c) 2020, AstraPowered by
...
if ( ! function_exists( 'astra_get_small_footer_custom_text' ) ) {
	...
	function astra_get_small_footer_custom_text( $option = '' ) {
		...
		if ( '' != $option ) {
			...
			$output = str_replace( '[site_title]',
                '<span class="ast-footer-site-title">' . get_bloginfo( 'name' ) .
                 '</span>', $output );
			...
			$output = str_replace( '[theme_author]', '<a href="' .
                esc_url( $theme_author['theme_author_url'] ) . '">' . $theme_author['theme_name'] .
                '</a>', $output );
		}
